...
{
	public partial class Glove : ViewController
	{
		void Start()
		{
			// Code Here
			Observable.EveryUpdate()
        	.Where(_=>(SelectToolsName.Instance().selectToolName.Equals("Glove")&&SelectToolsName.Instance().selectToolOpen))
			.Subscribe(_=>{
				UIKit.OpenPanel<UITitlePanel>();
				SelectToolsName.Instance().selectToolOpen=false;
				UIKit.GetPanel<UITitlePanel>().title.Value="一只手套";
        	});
		}
		...
		private void OnMouseDown() {
			...
			UIKit.GetPanel<UIMessagePanel>().text.Value="获得一只手套";
			...
			.Subscribe(_=>{
				...
				.Subscribe(destroy=>{
					//把钥匙放到UITools
					UIKit.GetPanel<UIToolsPanel>().collection.Add("Glove");
