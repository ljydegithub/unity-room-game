...
{
	public partial class IronKey : ViewController
	{
		void Start()
		{
			// Code Here
			Observable.EveryUpdate()
        	.Where(_=>(SelectToolsName.Instance().selectToolName.Equals("IronKey")&&SelectToolsName.Instance().selectToolOpen))
			.Subscribe(_=>{
				UIKit.OpenPanel<UITitlePanel>();
				SelectToolsName.Instance().selectToolOpen=false;
				UIKit.GetPanel<UITitlePanel>().title.Value="一把铁作的钥匙";
        	});
		}
		...
		private void OnMouseDown() {
			...
			UIKit.GetPanel<UIMessagePanel>().text.Value="获得一把铁钥匙";
			...
			.Subscribe(_=>{
				...
				.Subscribe(destroy=>{
					//把钥匙放到UITools
					UIKit.GetPanel<UIToolsPanel>().collection.Add("IronKey");
